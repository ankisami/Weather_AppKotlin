package training.android.myweather.model
...
private const val DATABASE_NAME = "MyWeather.db"
...
class Database(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    fun createCity(city: City): Boolean{
        ...
        content.put(CITY_KEY_NAME, city.name)

        city.id = writableDatabase.insert(CITY_TABLE_NAME, null, content)
        Log.d(TAG, "Création de la ville : ${city.name} dans la base de donnée.")
    ...
    fun deleteCity(city: City): Boolean {
        //Retourne le nombre de d'élément supprimé
        val deleteCount = writableDatabase.delete(
            CITY_TABLE_NAME, "$CITY_KEY_ID = ?",
            arrayOf("${city.id}")
        )
